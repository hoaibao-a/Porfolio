  ...
  return (
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-white shadow-md`}>
      <div className="w-full flex justify-between items-center max-w-7xl mx-auto">
        ...
        >
          <p className="text-light-orange text-[24px] font-bold cursor-pointer flex">
            Minh &nbsp;
            <span className="md:block hidden">Mai</span>
          </p>
        </Link>
            ...
            {navLinks.map((nav) => (
              <li
                ...
                className={`relative text-[16px] font-medium cursor-pointer ${
                  active === nav.id ? "text-light-orange" : "text-[#aaa6c3]"
                } hover:text-black`}
              ...
              >
                ...
                {/* Thanh gạch chân */}
                {active === nav.id && (
                  <span className="absolute left-0 bottom-[-5px] w-full h-[3px] bg-light-orange transition-all duration-300"></span>
                )}
              </li>
            ))}
          </ul>
